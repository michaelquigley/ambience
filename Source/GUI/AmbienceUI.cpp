...
void AmbienceLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int w, int h,
    ...
    juce::Slider& slider)
{
    ...
    juce::ColourGradient grad(AmbienceColors::AccentBlue, cx - r, cy, AmbienceColors::Accent, cx + r, cy, false);
...
void AmbienceLookAndFeel::drawLinearSlider(juce::Graphics& g, int x, int y, int w, int h,
...
void AmbienceLookAndFeel::drawComboBox(juce::Graphics& g, int w, int h, bool isDown, int, int, int, int, juce::ComboBox&)
{
...
void AmbienceLookAndFeel::drawGroupComponentOutline(juce::Graphics& g, int w, int h, const juce::String& text, const juce::Justification&, juce::GroupComponent&)
{
    ...
    g.drawText(text, (int)(indent + 14.f), 0, (int)tw, (int)textH, juce::Justification::centredLeft);
}
...
void RT60Visualizer::paint(juce::Graphics& g) {
    ...
    for (int i = 0; i < FDNReverb::NUM_BANDS; ++i) {
        ...
        g.drawText(fLbls[i], (int)(px - 12.f), (int)(y0 + H - 14.f), 24, 13, juce::Justification::centred);
    }
    ...
    auto plotCurve = [&](const std::array<float, FDNReverb::NUM_BANDS>& rt60, juce::Colour col, float thick) {
        juce::Path path;
        bool first = true;
        for (int i = 0; i < FDNReverb::NUM_BANDS; ++i) {
        ...
        g.setColour(col);
        g.strokePath(path, juce::PathStrokeType(thick, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
    ...
    if (processor) {
        ...
        plotCurve(preset.acoustics.rt60, AmbienceColors::TextSecondary.withAlpha(0.5f), 1.f);
    }
    ...
    g.setColour(AmbienceColors::TextSecondary);
    g.setFont(9.f);
    g.drawText("RT60 (s) per band", (int)x0 + 36, (int)y0 + 3, (int)W - 40, 12, juce::Justification::right);
}
// ─── VUMeter ────────────────────────────────────────────────────────────────
...
void VUMeter::paint(juce::Graphics& g) {
    ...
    auto bar = [&](float y, float level) {
        float n = juce::jlimit(0.f, 1.f, juce::jmap(juce::Decibels::gainToDecibels(level + 1e-9f), -60.f, 0.f, 0.f, 1.f));
        g.setColour(AmbienceColors::ArcTrack);
        g.fillRoundedRectangle(bx, y, bw, 7.f, 2.f);
        juce::ColourGradient gr(AmbienceColors::AccentBlue, bx, y, AmbienceColors::Accent, bx + bw, y, false);
    ...
    g.setColour(AmbienceColors::TextSecondary);
    g.setFont(8.f);
    g.drawText(label, (int)b.getX(), (int)b.getY(), 20, (int)b.getHeight(), juce::Justification::centredLeft);
}

// ─── ArcKnob ────────────────────────────────────────────────────────────────
void ArcKnob::build(juce::AudioProcessorValueTreeState& apvts, const juce::String& paramID, const juce::String& labelText, juce::Component* parent, AmbienceLookAndFeel& laf)
{
    slider.setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);
    slider.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 62, 14);
    slider.setLookAndFeel(&laf);
    slider.setColour(juce::Slider::textBoxTextColourId, AmbienceColors::TextSecondary);
...
// ─── AlgorithmSelector ──────────────────────────────────────────────────────
AlgorithmSelector::AlgorithmSelector(juce::AudioProcessorValueTreeState& a) : apvts(a)
{
    static const char* names[] = { "ROOM1","ROOM2","HALL1","HALL2","PLATE","SPRING","GOLDFOIL" };
    for (int i = 0; i < FDNReverb::NUM_ALGORITHMS; ++i) {
        ...
        buttons[i].onClick = [this, idx] {
            if (auto* param = apvts.getParameter("algorithm"))
                param->setValueNotifyingHost((float)idx / (float)(FDNReverb::NUM_ALGORITHMS - 1));
            };
    }
    apvts.addParameterListener("algorithm", this);
    currentAlgo = juce::roundToInt(*apvts.getRawParameterValue("algorithm") * (FDNReverb::NUM_ALGORITHMS - 1));
}

AlgorithmSelector::~AlgorithmSelector() { apvts.removeParameterListener("algorithm", this); }

void AlgorithmSelector::parameterChanged(const juce::String&, float newVal) {
    int newAlgo = juce::jlimit(0, FDNReverb::NUM_ALGORITHMS - 1, juce::roundToInt(newVal));
    juce::MessageManager::callAsync([this, newAlgo] { currentAlgo = newAlgo; updateButtonColors(); });
}

void AlgorithmSelector::updateButtonColors() {
    for (int i = 0; i < FDNReverb::NUM_ALGORITHMS; ++i) {
        bool on = (i == currentAlgo);
        buttons[i].setColour(juce::TextButton::buttonColourId, on ? AmbienceColors::Accent : AmbienceColors::Surface);
        buttons[i].setColour(juce::TextButton::textColourOffId, on ? AmbienceColors::Background : AmbienceColors::TextSecondary);
        buttons[i].repaint();
    }
}
